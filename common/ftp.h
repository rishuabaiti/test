...
namespace idc
{
    ...
    public:
        ...
        void Init(); // 初始化m_size和m_mtime成员变量。
        ...
        bool Login(const std::string &host, const std::string &username, const std::string &password, const int imode);
        ...
        bool getRemotefileMtime(const std::string &remotefilename);

        // 获取ftp服务器上文件的大小。
        // remotefilename：待获取的文件名。
        // 返回值：false-失败；true-成功，获取到的文件大小存放在m_size成员变量中。
        bool getRemotefileSize(const std::string &remotefilename);

        // 改变ftp服务器的当前工作目录。
        // remotedir：ftp服务器上的目录名。
        // 返回值：true-成功；false-失败。
        bool ChangeDir(const std::string &remotedir);
        ...
        bool getNameList(const std::string &remotedir, const std::string &locallistfilename);
        ...
        bool getFile(const std::string &remotefilename, const std::string &localfilename, const bool checkmtime = true);
        ...
        bool putFile(const std::string &localfilename, const std::string &remotefilename, const bool checksize = true);

        // 删除ftp服务器上的文件。
        // remotefilename：待删除的ftp服务器上的文件名。
        // 返回值：true-成功；false-失败。
        bool RemoveFile(const std::string &remotefilename);

        // 重命名ftp服务器上的文件。
        // srcremotefilename：ftp服务器上的原文件名。
        // dstremotefilename：ftp服务器上的目标文件名。
        // 返回值：true-成功；false-失败。
        bool RenameFile(const std::string &srcremotefilename, const std::string &dstremotefilename);
    ...
    private:
        netbuf *ftpconn; // ftp连接句柄。包含socket
    };
